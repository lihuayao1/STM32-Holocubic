...
void setup_scr_screen(lv_ui *ui){
	...
	lv_label_set_text(ui->screen_hour, "19");
	...
	lv_label_set_text(ui->screen_min, "20");
	...
	lv_label_set_text(ui->screen_second, "02");
	...
	lv_label_set_text(ui->screen_weather_info, "最高气温：35度，最低气温：20度，降水概率50%，相对湿度20%。");
	...
	lv_label_set_text(ui->screen_date, "08月27日");
...
//无法直接传中文字符串进来，原因同上
void updateDeskopCity(u8 cityName)	//采用传入数组下标的方式更新城市
{
	char cityNameArr[][10] = {"深圳","湛江","广州","赣州","南昌","北京","上海"};
	lv_label_set_text(lv_gui_deskop.screen_city,cityNameArr[cityName]);
}

void updateDeskopWeatherText(u8 weather)	//还需更新图标，后期再写。。。。。//////////////
{
	char weatherArr[][15] = {"晴","多云","阴","雨","雪","沙尘","雾","大风","台风","冷","热"};
	char str[25];	memset(str,0,sizeof(str));
	strcat(str,"天气：");	strcat(str,weatherArr[weather]);
...
void updateDeskopWeatherInfo(int temperatureHigh,int temperatureLow,u8 rainPercent,u8 humidityPercent)
{
	...
	strcat(str,"度，降水概率：");
	numToString(rainPercent,temp);	strcat(str,temp);	
	strcat(str,"%，相对湿度");

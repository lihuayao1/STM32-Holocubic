...
float formerPitch = 0,formerRoll = 0,formerYaw = 0;
float Pitch = 0,Roll = 0,Yaw = 0;
extern u32 msTicks;
u8 led = 0;
u8 updateEnable = 1;

//遗留bug：剧烈摇晃后，该函数由于等待回正，会出现异常
//修改方法：后续应修改为上电读取初始姿态，只需检测当前姿态与初始姿态夹角即可
void MPU6050_Task(void)	//姿态检测任务，执行时间约50ms
{
	MPU6050_DMP_Task();
	if(msTicks % 100 == 0 && updateEnable == 1)	//100ms时更新旧角度
	{
		formerPitch = Pitch;	formerRoll = Roll;	formerYaw = Yaw;
		updateEnable = 0;	//防止第200,300,400ms时也更新
	}
	if(msTicks % 500 == 0)											//500ms时检测是否出现大幅角度变化
	{
		if((Pitch - formerPitch) > 20)
		{
			while(formerPitch < Pitch-5) MPU6050_DMP_Task();//等待用户回正到100ms时的位置（-5为误差，无需完全恢复当时的位置，差不多即可）
			delay_ms(100);	
			printf("下摇!\r\n");
		}
		else if((Pitch - formerPitch) < -20)
		{
			while(Pitch < formerPitch-5) MPU6050_DMP_Task();
			delay_ms(100);	
			printf("上摇!\r\n");
		}
		else if((Roll - formerRoll) > 20)
		{
			while(formerRoll < Roll-5) MPU6050_DMP_Task();
			delay_ms(100);
			printf("右摇!\r\n");
		}
		else if((Roll - formerRoll) < -20)
		{
			while(Roll < formerRoll-5) MPU6050_DMP_Task();
			delay_ms(100);
			printf("左摇!\r\n");
		}
		updateEnable = 1;

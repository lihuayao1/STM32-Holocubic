...
//开窗，并利用DMA写任意长度数据（小于65535）。可写半帧，用于picture功能
...
void LCD_LVGL_ColorFill(u16 xStart,u16 yStart,u16 xEnd,u16 yEnd,u16* color)
{
	...
	LCD_DC_UP;
	LCD_CS_DOWN;	
	SPI1DMA_Init((u8*)color,(xEnd-xStart+1)*(yEnd-yStart+1)*2);//16位color传入8位spi寄存器，故大小*2
	DMA_ITConfig(DMA2_Stream3,DMA_IT_TC,ENABLE);
	SPI_I2S_DMACmd(SPI1,SPI_I2S_DMAReq_Tx,ENABLE);
	DMA2_COMPLETED_LCD = 0;
	while(DMA2_COMPLETED_LCD == 0);
...
{
		static u32 lastGifTicks = 0;	//记录上一次更新gif的时间
		static u8 i = 0;
		if(msTicks - lastGifTicks > 80)	//距离上一次更新80ms以上，则刷新图
		{
			updateDeskopDancePattern(i++);
			lastGifTicks = msTicks;
		}
		if(i == 198)
			i = 0;
}
